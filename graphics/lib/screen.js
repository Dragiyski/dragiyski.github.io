...

{
    ...
    const resourceNames = names.filter(name => {
        ...
        let resourceName = name.substr(stringCreate.length);
    ...
    for (const name of resourceNames) {
        let resourceName = name.substr(stringCreate.length);
        let keyName = resourceName.charAt(0).toLowerCase() + resourceName.substr(1);
...
/**
 * TODO:
 * To save the neutral state, we must save the state and associate a context to internal object which has the storage.
 * Beside the storage, it will have an error (initially `null`) and resources (initially empty object).
 *
 * The `gl.create*` function that has corresponding `gl.delete*` and `gl.is*` functions and:
 * `gl.fenceSync` with `gl.isSync` and `gl.deleteSync` must be overridden, storing the
 * returned objects in the current context appropriate resource.
 *
 * In case of exception from OpenGLContext callback, the state reset the context. To reset the context:
 * * All collected resource object must be checked with the corresponding `is*` function and if `true`, must be deleted with the `delete*` function.
 * * All variable resource sizes must be freed with the appropriate MAX_* parameter. For example GL_TEXTURE0 + i for all i from 0 to
 * `gl.getParameter(gl.MAX_COMBINED_TEXTURE_IMAGE_UNITS), all binding points must be unbound.
 * * All global state must be reset to parameters collected in the initial (neutral) state.
 *
 * The context will also be reset upon switching a context with the following policy:
 * * If the current context is `null` no reset occurs.
 * * If the current context is not `null` reset occurs.
 *
 * The reset guarantees the the state is not leaked into the new context. We cannot trust onStop no modify the global state. Note, that reset on context switch
 * MUST NOT free any resources created by the previous context (unless exception occurs). Resources will be preserved until release() is called for that
 * context or page is unloaded.
 */
