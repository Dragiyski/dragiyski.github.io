function createProgramFromSources(
    gl,
    { source: vertexSource, files: vertexFiles },
    { source: fragmentSource, files: fragmentFiles }
) {
    if (!(gl instanceof WebGL2RenderingContext)) {
        throw new TypeError(`Invalid "gl": expected [object WebGL2RenderingContext], got ${getType(gl)}`);
    }
    if (typeof vertexSource !== 'string') {
        throw new TypeError(`Invalid "vertexSource": expected [string], got ${getType(vertexSource)}`);
    }
    if (!Array.isArray(vertexFiles) || vertexFiles.length <= 0) {
        throw new TypeError(`Invalid "vertexFiles": expected [object Array], got ${getType(vertexFiles)}`);
    }
    if (typeof fragmentSource !== 'string') {
        throw new TypeError(`Invalid "fragmentSource": expected [string], got ${getType(fragmentSource)}`);
    }
    if (!Array.isArray(fragmentFiles) || fragmentFiles.length <= 0) {
        throw new TypeError(`Invalid "fragmentFiles": expected [object Array], got ${getType(fragmentFiles)}`);
    }
    ...
    try {
        ...
        if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
            throw new ShaderCompileError(
                processErrorString(gl.getShaderInfoLog(vertexShader), vertexFiles),
                { context: gl, shaderSource: vertexSource, shaderType: 'VERTEX_SHADER' }
            );
        }
        ...
        if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
            throw new ShaderCompileError(
                processErrorString(gl.getShaderInfoLog(fragmentShader), fragmentFiles),
                { context: gl, shaderSource: fragmentSource, shaderType: 'FRAGMENT_SHADER' }
            );
        }
        ...
        if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            throw new ShaderProgramLinkError(
                gl.getProgramInfoLog(program),
                { context: gl, source: { vertex: vertexSource, fragment: fragmentSource } }
            );
        }
...
function processErrorString(log, files) {
    return log.split(/(?:\r?\n|\r)/mg).map(line => {
        ...
        const lineIndex = parseInt(match[3], 10);
        if (fileIndex < 0 || fileIndex >= files.length) {
            return line;
        }
        return `${match[1]} ${files[fileIndex]}:${lineIndex}: ${match[4]}`;
    }).join('\n');
}
